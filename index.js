...
let winplayer1 = false, winplayer2 = false;
...
function checkVertically(grid, row, col) {
    ...
    for(i=0; i<=delrow.length-4; i++) {
        if(delrow[i]==1 && delrow[i+1]==1 && delrow[i+2]==1 && delrow[i+3]==1) {
            winplayer1 = true;
            break;
        }
        else if(delrow[i]==2 && delrow[i+1]==2 && delrow[i+2]==2 && delrow[i+3]==2) {
            winplayer2 = true;
            break;
        }
    }
...
function checkHorizontally(grid, row, col) {
    ...
    for(i=0; i<=delcol.length-4; i++) {
        if(delcol[i]==1 && delcol[i+1]==1 && delcol[i+2]==1 && delcol[i+3]==1) {
            winplayer1 = true;
            break;
        }
        else if(delcol[i]==2 && delcol[i+1]==2 && delcol[i+2]==2 && delcol[i+3]==2) {
            winplayer2 = true;
            break;
        }
    }
...
function checkLeftDiagonals(grid) {
    for(i=0; i<6; i++) {
        for(j=0; j<7; j++) {
            ...
            if(i<(6-3) && j<(7-3) && grid[i][j]==1 && grid[i+1][j+1]==1 && grid[i+2][j+2]==1 && grid[i+3][j+3]==1) {
                winplayer1 = true;
                break;
            }

            if(i<(6-3) && j<(7-3) && grid[i][j]==2 && grid[i+1][j+1]==2 && grid[i+2][j+2]==2 && grid[i+3][j+3]==2) {
                winplayer2 = true;
                break;
            }
        }
...
function checkRightDiagonals(grid) {
    for(i=0; i<6; i++) {
        for(j=0; j<7; j++) {
            ...
            if(i<(6-3) && j>=3 && grid[i][j]==1 && grid[i+1][j-1]==1 && grid[i+2][j-2]==1 && grid[i+3][j-3]==1) {
                winplayer1 = true;
                break;
            }

            if(i<(6-3) && j>=3 && grid[i][j]==2 && grid[i+1][j-1]==2 && grid[i+2][j-2]==2 && grid[i+3][j-3]==2) {
                winplayer2 = true;
                break;
            }
        }
...
function interfaceCreation(grid, visited) {
    ...
    rl.question(`Enter the column for Player${playerName}: `, (col) => {
        if(col >= 0 && col<7) {
            ...
            console.log(`Player${playerName}, Please enter a correct column between 0 to 7` + "\n");
            ctr--;
            interfaceCreation(grid, visited);
        }
    });
